...
void compute_energy_matrix(const Image* img, Matrix* energy) {
    ...
    for (int row = 0; row < Matrix_height(energy); ++row) {
        for (int col = 0; col < Matrix_width(energy); ++col) {
            if (col != 0 && col != Matrix_width(energy) - 1 && row != 0 &&
                row != (Matrix_height(energy) - 1)) {
...
void compute_vertical_cost_matrix(const Matrix* energy, Matrix* cost) {
    ...
    for (int j = 1; j < Matrix_height(cost); ++j) {
        for (int x = 0; x < Matrix_width(cost); ++x) {
            if (x == 0) {
                int val = *Matrix_at(energy, j, x) +
                    Matrix_min_value_in_row(cost, j - 1, x, x + 2);
                *Matrix_at(cost, j, x) = val;
            }
            else if (x == Matrix_width(cost) - 1) {
                int val = *Matrix_at(energy, j, x) +
                    Matrix_min_value_in_row(cost, j - 1, x - 1, x + 1);
                *Matrix_at(cost, j, x) = val;
            }
            else {
                int val = *Matrix_at(energy, j, x) +
                    Matrix_min_value_in_row(cost, j - 1, x - 1, x + 2);
                *Matrix_at(cost, j, x) = val;
            }
        }
    }
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
    for (int i = Matrix_height(cost) - 1; 0 <= i; --i) {
        if (i == Matrix_height(cost) - 1) {
            seam[i] = Matrix_column_of_min_value_in_row(cost, i, 0,
                                                        Matrix_width(cost));
        }
        else {
            if (seam[i + 1] == 0) {
                seam[i] = Matrix_column_of_min_value_in_row(cost, i, seam[i + 1],
                    seam[i + 1] + 2);
            }
...
void remove_vertical_seam(Image* img, const int seam[]) {
    ...
    for (int i = 0; i < Image_height(img); ++i) {
        ...
        for (int j = 0; j < Image_width(img); ++j) {
            if (j != seam[i]) {
                Image_set_pixel(new_image, row, col, Image_get_pixel(img, i, j));
                col++;
            }
        }
        row++;
